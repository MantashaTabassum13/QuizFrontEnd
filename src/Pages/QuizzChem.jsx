...
function QuizzPhy() {
    ...
    const changeQuestion = () => {
        updateScore();
        if (currentQuestion < QuizDataChem.length - 1) {
    ...
    const updateScore = () => {
        if (clickedOption === QuizDataChem[currentQuestion].answer) {
            setScore(score + 1);
        }
    }
    const resetAll = () => {
        ...
        <div>
            ...
            <div className='container mx-auto text-center  shadow-xl ' style={{marginTop:'-100px', backgroundColor:'white',borderRadius:'20px',backfaceVisibility:'0.2'}}>
                ...
                <div className="container">
                    {showResult ? (
                        <QuizResult score={score} totalScore={QuizDataChem.length} tryAgain={resetAll} />
                    ) : (
                        <>
                            <div className="question  text-2xl pb-10 font-semibold">
                                <span id="question-number">{currentQuestion + 1}. </span>
                                <span id="question-txt">{QuizDataChem[currentQuestion].question}</span>
                            </div>
                            <div className="container items-center  gap-x-20 grid gap-4 md:grid-cols-2 sm:grid-cols-1 sm:text-m md:text-xl font-medium px-10">
                                {QuizDataChem[currentQuestion].options.map((option, i) => {
                                    return (
